(() => {
if(window.__BLOOKET_LAN_CHAT__) return; window.__BLOOKET_LAN_CHAT__ = true;
...
css.textContent = `
#chatBox{position:fixed;top:50px;left:50px;width:90%;max-width:420px;height:60%;max-height:520px;background:linear-gradient(180deg,#4a90e2,#357ABD);border:3px solid #333;border-radius:12px;display:flex;flex-direction:column;box-shadow:0 5px 20px rgba(0,0,0,0.3);z-index:999999;font-family:sans-serif;}
#header{padding:12px;background:#357ABD;cursor:move;display:flex;justify-content:space-between;font-weight:bold;border-radius:9px 9px 0 0;color:#fff;touch-action:none;}
#rooms{display:flex;justify-content:space-between;margin:4px;}
...
#inp{flex:1;border:none;padding:10px;outline:none;border-top:1px solid #333;background:#e6f0ff;}
#sendBtn{padding:10px;background:#9b30ff;color:#fff;border:none;border-radius:0 0 9px 9px;cursor:pointer;}
...
function startDrag(x, y) { drag = true; ox = x - chatBox.offsetLeft; oy = y - chatBox.offsetTop; }
function moveDrag(x, y) { if(drag){ chatBox.style.left = (x - ox) + "px"; chatBox.style.top = (y - oy) + "px"; } }
function endDrag() { drag = false; }

header.addEventListener("mousedown", e => startDrag(e.clientX, e.clientY));
document.addEventListener("mousemove", e => moveDrag(e.clientX, e.clientY));
document.addEventListener("mouseup", endDrag);
header.addEventListener("touchstart", e => { e.preventDefault(); const t = e.touches[0]; startDrag(t.clientX, t.clientY); });
...
document.querySelectorAll("#rooms button").forEach(b => {
  b.onclick = () => {
    document.querySelectorAll("#rooms button").forEach(x => x.classList.remove("on"));
    b.classList.add("on");
    room = b.dataset.r;
    msgs.innerHTML = "";
  };
...
dcs["self"] = dc; pcs["self"] = pc;
dc.onmessage = e => show(JSON.parse(e.data));
pc.onicecandidate = e => { if(e.candidate) ws.send(JSON.stringify({ type: "ice", candidate: e.candidate, from: peerId })); };
pc.createOffer().then(offer => pc.setLocalDescription(offer).then(() => ws.send(JSON.stringify({ type: "offer", offer, from: peerId }))));

// Input events: Enter + Send button (mobile-friendly)
function sendInput(){ if(!inp.value) return; send({ u: user, m: inp.value, t: Date.now(), room }); inp.value = ""; }
inp.addEventListener("keydown", e => { if(e.key === "Enter") sendInput(); });
